<template>
  <div v-if="permission">
    <v-btn
      ...
      :class="[{ 'icon-right': !left },{'vc-button-custom-style': schema.revampSchema && schema.revampSchema[0] && schema.revampSchema[0].members},schema.classes, 'mx-1', 'btndefault']"
      :type="type"
      :style="[schema.style,computeStyle('',schema)]"
      :href="schema.href"
      :to="schema.to"
      :target="schema.target || '_blank'">
...
<script>
  ...
  export default {
    ...
    methods: {
      click() {
        return this.messageDialog.showMsg({
          title: "",
          message: this.$t("vc.form.recoverConfirmMessage"),
          mode: "confirm",
          fn: this.recoverfun,
          fnCancel: this.recoverCancel,
        });
      },
      recoverfun() {
        let self = this;
        self.$axios
          .post(
            ...
            {
              requestURL: `${self.pageView}/doc/recover/${self.$route.params.id}/${self.schema.name}`
            }
          )
          .then(({ data }) => {
            if (data.error) {
              ...
              switch (data.error.code.toLowerCase()) {
                ...
                case "document_notallowed":
                  ...
                  break;
                default:
                  errorMsg = self.$t("error.recoverErrorMessage");
                  break;
              }
              this.messageDialog.showMsg({ title: self.$t("vc.form.error"), message: errorMsg});
            } else {
              return self.messageDialog.showMsg({
                title: "",
                message: self.$t("success.successfullyRecover"),
                mode: "confirm",
                fn: self.recoverSuccess,
                fnCancel: self.recoverCancel,
              });
      ...
      recoverSuccess() {
        if(window.urlMode == 'NOTENANTAPPCODE') {
          ...
          window.location.href = "/" + app.tenantId + "/" + app.appCode;
        }
      },
      filterRecoverPermission() {
        let self = this;
        ...
        //如果用户没有设置自定义权限，则按照系统内置权限来处理Recover Button
        if (permissions.length == 0) {
          //依据IsDeleted状态判断是否为true, 且必须满足是AppAdmin
          if (self.model["Ng_SysIsDeleted"] && self.model["Ng_SysIsDeleted"] == true && self.$store.state.userInfo.roles.indexOf("AppAdmin") > 0) {
    ...
    computed: {
      ...
      showBtn() {
        return this.schema.show && this.$route.name !== "Add";
      },
      left() {
    ...
    mounted() {
      this.filterRecoverPermission();
    }
  };
</script>
<style>
.v-application .recover-btn.vc-button-custom-style {
